...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rateCheck = await checkPersistentRateLimit(base44, user.email, 'createFanClubCheckout', 5, 86400000);
    ...
    const idempotencyKey = await generateIdempotencyKey(user.email, `fanclub_${creator_id}_${tier}`, price_usd);
    const idempotencyCheck = await checkIdempotency(base44, idempotencyKey);
    if (idempotencyCheck.isDuplicate) return Response.json({ duplicate: true, originalId: idempotencyCheck.originalId, message: 'Duplicate fan club checkout' }, { status: 409 });
