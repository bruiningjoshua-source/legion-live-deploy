...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (users[0]?.isSuspended) {
      return Response.json({ error: `Account suspended: ${users[0].suspensionReason || 'Policy violation'}` }, { status: 403 });
    }
    ...
    const rateCheck = await checkPersistentRateLimit(base44, user.email, 'createTipCheckout', 5, 60000);
    if (!rateCheck.allowed) {
      return Response.json({ error: 'Rate limited: 5 tips per minute', retryAfter: rateCheck.retryAfter }, { status: 429 });
    }

    // ── Idempotency check ──
    const idempotencyKey = await generateIdempotencyKey(user.email, 'tip', amount);
    const idempotencyCheck = await checkIdempotency(base44, idempotencyKey);
    if (idempotencyCheck.isDuplicate) {
      return Response.json({ duplicate: true, originalId: idempotencyCheck.originalId, message: 'Duplicate tip checkout' }, { status: 409 });
    }
