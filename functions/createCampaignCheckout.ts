...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const idempotencyKey = await generateIdempotencyKey(user.email, `campaign_${campaignId}`, amount);
    const idempotencyCheck = await checkIdempotency(base44, idempotencyKey);
    if (idempotencyCheck.isDuplicate) return Response.json({ duplicate: true, originalId: idempotencyCheck.originalId, message: 'Duplicate campaign checkout' }, { status: 409 });
